...
function deliver_response($format, $api_response){
	...
	//if( strcasecmp($format) == '')$format = 'json';
...
if( strcasecmp($_GET['method'],'Learzing') == 0){
    $response['code'] = 1;
    $response['status'] = $api_response_code[ $response['code'] ]['HTTP Response'];
    $response['data'] = 'This is a model RESTful API webservice method. Currently returning a dump data to check JSon response. Will need to plug in the database (Postgress SQL) calls sproc for friend finding.';
}

// Method B: Search for the friends API
if( strcasecmp($_GET['method'],'LearzingFriends') == 0){
    
   
    $response['code'] = 1;
    $response['status'] = $api_response_code[ $response['code'] ]['HTTP Response'];
    $response['data'] = vbmtesting(); //'This is a model RESTful API webservice method. Currently returning a dump data to check JSon response. Will need to plug in the database (Postgress SQL) calls sproc for friend finding.';
}
